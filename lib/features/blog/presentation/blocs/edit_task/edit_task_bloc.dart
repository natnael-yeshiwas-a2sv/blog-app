...
import 'edit_task_formfields.dart';
part 'edit_task_state.dart';

class EditTaskCubit extends Cubit<EditTaskState> {
  EditTaskCubit({required this.getTagsUsecase, required this.articleRepository})
      : super(EditTaskState(
            tags: const <String>[], selectedTags: const <String>[]));
  GetTags getTagsUsecase;
  ArticleRepository articleRepository;
  void init(Article article) {
    emit(state.copyWith(
        title: TitleInput.dirty(article.title),
        subtitle: SubtitleInput.dirty(article.subTitle),
        content: ContentInput.dirty(article.content),
  ...
  void fetchTags() async {
    ...
    res.fold((l) => null,
        (r) => emit(state.copyWith(status: Status.stable, tags: r)));
  }
    ...
    if (title.isValid &&
      ...
      await articleRepository
          ...
          .then((value) => value.fold(
              (l) => emit(state.copyWith(status: Status.submitFailed)),
      ...
      emit(state.copyWith(status: Status.stable));
    }
  }
}
